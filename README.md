...
- Initialize `SentimentPlotter` with `output_sentiment_csv`.
- `self.__sentiment_data = pd.read_csv(self.__output_sentiment_csv)` read the existing CSV file into a Pandas DataFrame.
- `self.__sentiment_data["datetime"] = pd.to_datetime(self.__sentiment_data["datetime"])` convert datetime column to datetime format.
- `self.__filtered_data = self.__sentiment_data[self.__sentiment_data["compound"] != 0]` remain comound that is not 0.
...
- `plt.figure(figsize=(1.618 * 8, 8))` set graph size.
- `plt.scatter(self.__filtered_data["datetime"], self.__filtered_data["compound"], c="k", alpha=0.3)` scatter x-axis (`datetime`) and y-axis (`compound`), color black, transparency 30%.
- `daily_com_avg = self.__filtered_data.groupby(self.__filtered_data["datetime"].dt.date)["compound"].mean()` group sentiment data by date and calculate average compound score.
- `plt.plot(daily_com_avg.index, daily_com_avg.values, "r-")` plot daily average sentiment as a red line.
- `plt.title(f"Index of {search_word}", fontsize=12)` set graph title and font size.
- `plt.ylabel("Score", fontsize=10)` set y label and font size.
- `plt.ylim(-1, 1)` set y axis limit from -1 to 1.
- `plt.grid(True)` set grid.
- `plt.xticks(rotation=60, fontsize=7)` rotate 60 degrees and font size.
- `png_dir = "png/" if not os.path.exists(png_dir): os.makedirs(png_dir)` set directory for saving plot png.
- `plot_filename = f"sentiment_plot_{search_word}.png"` set file name.
- `plot_filepath = os.path.join(png_dir, plot_filename)` set file path.
- `plt.savefig(plot_filepath)` save plot png to file path.
